...
// framebuffer/palette
static uint8_t frame_buffer[640 * 200];
static uint16_t temp_buffer[DPI_MODE_H_ACTIVE_PIXELS * DPI_NUM_DMA_CHANNELS];
...
static inline void convert_paletted(const uint8_t *in, uint16_t *out, int count) {
    ...
    {
        auto px = *in++;
        *out++ = cga_palette[px & 0xF];
        *out++ = cga_palette[px >> 4];
    }
}
...
bool display_in_first_half() {
    return started && timing_scanline < MODE_V_TOTAL_LINES / 2;
}
