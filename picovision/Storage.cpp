...
bool storage_init() {
  ...
  if(res == 5) {
    ...
    printf("CMD8 failed (res %X)\n", res);
    return false;
  } else if(data != 0x1AA) {
    printf("CMD8 returned unexpected %X!\n", data);
    return false;
  }
  ...
  while(res != 0) {
    ...
    if(res == 0x1)
      ...
      printf("CMD55 failed (res %X)\n", res);
      return false;
    }

    if(res > 1 && res != 0xFF) {
      printf("ACMD41 failed (res %X)\n", res);
      return false;
    }
  }
